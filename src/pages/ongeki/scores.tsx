...
const OngekiScorePage = () => {
  ...
  const columns = {
    Song: (row: OngekiScore) => (
      <div className="flex items-center gap-3">
        <img
          width={40}
          height={40}
          src={`${CDN}/ongeki/jacket/${row.jacketPath}`}
    ...
    "Platinum Stars": (row: OngekiScore) =>
      (row.platinumScoreStar ?? 0) > 0 && (
        <>
          ...
          <span className="ml-1">
            {row.platinumScoreStar?.toLocaleString()}
          </span>
        </>
      ),
    ...
    Difficulty: (row: OngekiScore) =>
      getDifficultyFromOngekiChart(row.chartId ?? 0),
    ...
    Playdate: (row: OngekiScore) =>
      row.userPlayDate
        ? new Date(row.userPlayDate).toLocaleString()
        : "Unknown",
  };
