...
const OngekiRatingFrames = () => {
  ...
  const handleSearch =
    (key: keyof typeof searchQueries) => (search: { value?: string }) => {
  ...
  const ratingColumns = {
    ...
    Rate: (row: OngekiRatingData) =>
      ratingTableColumns.Rate(row, isRefreshOrAbove),
  };
  ...
  const currentFumensTitle = isRefreshOrAbove
    ? "Top 10 current fumens"
    : "Recent 10 current fumen";
  ...
  const filteredRecommendedFumens = filterData(
    recommendedFumens,
    searchQueries.next
  );
      ...
      {version ? (
        <div className="container mx-auto space-y-6">
                    ...
                    {isRefreshOrAbove ? (
                      <>
                        ...
                        <span className="text-primary font-bold">
                          Highest Rating:{" "}
                          {(
                            (newHighestRating[0]?.newHighestRating ?? 0) / 1000
                          ).toFixed(3) || "Loading..."}
                        </span>
                      </>
                    ) : (
                      <>
                        <span className="text-primary font-bold">
                          Player Rating:{" "}
                          {((playerRating[0]?.playerRating ?? 0) / 100).toFixed(
                            2
                          ) || "Loading..."}
                        </span>
                        <span className="text-primary font-bold">
                          Highest Rating:{" "}
                          {(
                            (highestRating[0]?.highestRating ?? 0) / 100
                          ).toFixed(2) || "Loading..."}
                        </span>
                      </>
                    )}
          ...
          <ViewToggle
            ...
            onToggle={() =>
              setDisplayMode(displayMode === "table" ? "grid" : "table")
        ...
        <div className="flex h-[calc(100vh-64px)] items-center justify-center">
          <p className="text-primary">
            Please set your Ongeki version in settings first
          </p>
        </div>
      )}
    </div>
