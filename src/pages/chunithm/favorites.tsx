...
const ChunithmFavorites = () => {
  ...
  const { data: favoriteSongIds = [], isLoading: isLoadingFavorites } =
    useFavorites();
  ...
  const columns = {
    ...
    Title: (row: ChunithmFavorite) => (
      <span className="text-primary truncate">{row.title}</span>
    ),
    Favorite: (row: ChunithmFavorite) => {
      const isFavorited = favoriteSongIds.some(
        (favorite: ChunithmFavorite) => favorite.favId === row.songId
      );
      return (
        <Heart
