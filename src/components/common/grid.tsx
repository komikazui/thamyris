...
const GridComponent: React.FC<GridProps<any>> = ({
  data,
  onSearch,
  title,
  renderItem,
    ...
    <div className="bg-card w-full rounded-md p-2 sm:p-4">
      ...
      >
        {data.length > 0 ? (
          ...
          <div className="text-primary col-span-full py-8 text-center">
            No data available
          </div>
        )}
      </div>
            ...
            {item.hasTechScore && (
              <div className="flex flex-col">
                ...
                <span className="text-primary">
                  {item.techScoreMax?.toLocaleString() ?? "-"}
                </span>
              </div>
            )}
            ...
            {item.hasStars && item.platinumScoreStar > 0 && (
              <div className="flex flex-col">
                ...
                <div className="flex flex-wrap items-center">
                  {Array.from(
                    { length: Math.min(item.platinumScoreStar, 5) },
                    (_, i) => (
          ...
          <>
            ...
            {item.hasLamp && getChunithmComboStatus && (
              <div className="flex flex-col">
                ...
                {(() => {
                  const comboStatus = getChunithmComboStatus(
                    item.isFullCombo ?? 0,
                    item.isAllJustice ?? 0
                  );
                  if (!comboStatus)
            ...
            {item.hasRating && getChunithmRating && (
              <div className="flex flex-col">
                ...
                <span className="text-primary">
                  {(
                    (getChunithmRating(item.level!, item.score!) ?? 0) / 100
                  ).toFixed(2)}
                </span>
              </div>
            )}
