...
const NameplateSelector = () => {
  ...
  const { data: currentNameplate, isLoading: isLoadingCurrent } =
    useCurrentNameplates();
  ...
  const hasChanges = () => {
    const currentPath =
      Array.isArray(currentNameplate) && currentNameplate.length > 0
        ? currentNameplate[0].imagePath
        : "";
  ...
  const handleSubmit = () => {
    const selected = nameplates?.find(
      (nameplate) => nameplate.imagePath === selectedNameplate
    );
  ...
  return (
    <div className="flex w-full flex-col justify-center gap-4 px-4 pt-4 md:flex-row md:gap-8 md:pt-15">
      <div className="flex items-center justify-center self-center md:h-full md:w-[300px]">
        <img
          src={`${CDN}/chunithm/nameplate/${selectedNameplate.replace(".dds", ".png")}`}
