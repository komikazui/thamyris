...
const SystemvoiceSelector = () => {
  ...
  const { data: currentVoice, isLoading: isLoadingCurrent } =
    useCurrentSystemVoice();
  ...
  const hasChanges = () => {
    const currentPath =
      Array.isArray(currentVoice) && currentVoice.length > 0
        ? currentVoice[0].imagePath
        : "";
  ...
  const handleSubmit = () => {
    const selected = systemVoices?.find(
      (voice) => voice.imagePath === selectedVoice
    );
  ...
  return (
    <div className="flex w-full flex-col justify-center gap-4 px-4 pt-4 md:flex-row md:gap-8 md:pt-15">
      <div className="flex items-center justify-center self-center md:h-full md:w-[300px]">
        <img
          src={`${CDN}/chunithm/system_voice/${selectedVoice.replace(".dds", ".png")}`}
