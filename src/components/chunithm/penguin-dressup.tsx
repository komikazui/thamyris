...
const PenguinDressup = () => {
  const { data: currentAvatar, isLoading: isLoadingCurrent } =
    useCurrentAvatar();
  const { data: availableAccessories, isLoading: isLoadingParts } =
    useAllAvatarParts();
  ...
  const getAccessoryLabel = (part: string) => {
    const selectedValue =
      selectedAccessories[part as keyof typeof selectedAccessories];
    const accessory =
      availableAccessories?.[part as keyof typeof availableAccessories] || [];
    const selectedAccessory = accessory.find(
      (accessory) => accessory.image === selectedValue
    );
  ...
  const hasChanges = () => {
    ...
    return Object.keys(selectedAccessories).some(
      (key) =>
        ...
        currentAvatar[key as keyof typeof currentAvatar]
    );
  };
  ...
  const handleSubmit = () => {
    ...
    const avatarParts: Partial<AvatarParts> = {
      head:
        availableAccessories.head?.find(
          (item) => item.image === selectedAccessories.head
        )?.avatarAccessoryId || 0,
      face:
        availableAccessories.face?.find(
          (item) => item.image === selectedAccessories.face
        )?.avatarAccessoryId || 0,
      back:
        availableAccessories.back?.find(
          (item) => item.image === selectedAccessories.back
        )?.avatarAccessoryId || 0,
      wear:
        availableAccessories.wear?.find(
          (item) => item.image === selectedAccessories.wear
        )?.avatarAccessoryId || 0,
      item:
        availableAccessories.item?.find(
          (item) => item.image === selectedAccessories.item
        )?.avatarAccessoryId || 0,
    };
  ...
  return (
    <div className="z-0 flex w-full flex-col justify-center gap-4 px-4 pt-4 md:flex-row md:gap-8 md:pt-15">
      <div className="relative mb-6 h-[300px] w-full md:mb-0 md:h-[400px] md:w-[300px]">
        <div className="avatar_base relative h-[400px] w-[300px]">
          <div className="avatar_back">
            <img
              loading="lazy"
              src={`${CDN}/chunithm/avatar/${selectedAccessories.back}.png`}
            />
          </div>
          <div className="avatar_wear">
            <img
              src={`${CDN}/chunithm/avatar/${selectedAccessories.wear}.png`}
            />
          </div>
          <div className="avatar_skin">
            <img
              src={`${CDN}/chunithm/avatarStatic/CHU_UI_Avatar_Tex_01400001.png`}
            />
          </div>
          <div className="avatar_hand_l">
            <img
              src={`${CDN}/chunithm/avatarStatic/CHU_UI_Avatar_Tex_LeftHand.png`}
            />
          </div>
          <div className="avatar_hand_r">
            <img
              src={`${CDN}/chunithm/avatarStatic/CHU_UI_Avatar_Tex_RightHand.png`}
            />
          </div>
          <div className="avatar_head">
            <img
              src={`${CDN}/chunithm/avatar/${selectedAccessories.head}.png`}
            />
          </div>
          <div className="avatar_face_static">
            <img
              src={`${CDN}/chunithm/avatarStatic/CHU_UI_Avatar_Tex_Face.png`}
            />
          </div>
          <div className="avatar_face">
            <img
              src={`${CDN}/chunithm/avatar/${selectedAccessories.face}.png`}
            />
          </div>
          <div className="avatar_item_l">
            <img
              src={`${CDN}/chunithm/avatar/${selectedAccessories.item}.png`}
            />
          </div>
      ...
      <div className="bg-card w-full rounded-md p-4 md:w-[400px] md:p-6">
        {Object.entries(availableAccessories || {}).map(
          ([category, options]) => (
            <AvatarDropdown
              key={category}
              category={category}
